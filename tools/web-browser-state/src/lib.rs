//! Read and change browser state: cookies, localStorage, sessionStorage, dialogs and the viewport size.
//!
...
impl Guest for Component {
    fn describe() -> ToolManifest {
        ToolManifest {
            ...
            description: "Read and change browser state: cookies, localStorage, sessionStorage, dialogs and the viewport size.".to_string(),
    ...
    /// `config_json` is this tool's own `[tools.web-browser-state]` block from
    ...
    fn invoke(
        ...
        sys::log(LogLevel::Debug, &format!("web-browser-state invoked with: {input}"));

        // Replace this with the real implementation.
        Ok(format!("web-browser-state is a stub; it received: {input}"))
    }
}
