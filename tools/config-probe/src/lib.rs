//! Reports the settings this tool was handed
//!
...
impl Guest for Component {
    fn describe() -> ToolManifest {
        ToolManifest {
            name: "config-probe".to_string(),
            description: "Reports the settings this tool was handed".to_string(),
    ...
    /// `config_json` is this tool's own `[tools.config-probe]` block from
    ...
    fn invoke(
        ...
        sys::log(LogLevel::Debug, &format!("config-probe invoked with: {input}"));

        // Reports the block rather than a stub, so the settings a tool is handed
        // can be seen from the chat.
        Ok(format!(
            "input: {input}
my settings: {}",
            serde_json::to_string_pretty(&config).unwrap_or_else(|_| config_json.clone())
        ))
    }
}
