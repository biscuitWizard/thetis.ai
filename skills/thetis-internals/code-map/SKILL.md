...
A checkout may carry local modules this map does not name — a gateway, its
tools, its skills and its `conf.d/` fragment, each in directories of its own
carrying a `.thetis-private` marker. They are not part of the kernel and
nothing here depends on them; read the marker in a directory you do not
recognise before assuming it is shared.
