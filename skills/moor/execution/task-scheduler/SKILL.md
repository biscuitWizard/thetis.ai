...
when_to_use = "Use when the question is about task lifecycle in the mooR server: what starts a task, suspend() and read() and fork and resume() and kill_task(), queued versus active tasks, background tasks, task ids, wait_task, task_send and task_recv, transaction conflict retry and backoff, tasks surviving a restart, checkpoints and dump_interval, or the fg_ticks, bg_ticks, fg_seconds, bg_seconds, max_stack_depth and max_task_retries limits on $server_options. Do not use it for opcode execution or the activation stack, for writing a builtin, for permission checks, or for command parsing. Do not use it for the database transaction protocol itself, for the Torchship MOO game database, for in-world MOO verb authoring, or for Thetis's own internals."
universal = false
tags = ["moor", "scheduler", "task", "suspend", "fork", "resume", "kill_task", "ticks", "server_options", "conflict retry", "background task", "checkpoint", "queued_tasks", "task_recv"]
version = 1
---
