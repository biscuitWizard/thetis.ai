...
when_to_use = "Use for work in mooR's database layer: the moor-db crate, the WorldState and LoaderInterface traits, CommitResult::ConflictRetry and commit conflicts, relations, property and verb resolution, caches, snapshots and checkpoints, the fjall key-value store, object create and recycle, anonymous objects, and garbage collection. Use it to choose the child skill. Not for MOO language or compiler questions, the scheduler's own queues and limits, the objdef or textdump file formats, the Torchship game database (the torchship skills own that), in-world verb authoring for a specific game, or Thetis's own internals."
universal = false
tags = ["moor", "moo", "database", "transactions", "worldstate", "moor-db", "mvcc", "conflict", "retry", "objects", "properties", "verbs", "fjall", "snapshot", "checkpoint", "garbage collection", "storage"]
children = "auto"
...
| Question | Read |
|---|---|
| Why does my commit conflict? What does a retry re-run? | `transactions` |
| How are objects, properties, verbs and inheritance stored? | `world-state-model` |
| Who checks permissions? What must a caller supply? | `world-state-model` |
| What is in memory, what is on disk, what survives a crash? | `storage-engine` |
| What is a snapshot, a checkpoint, a migration? | `storage-engine` |
| How is an object created, recycled, or collected? | `object-lifecycle-and-gc` |
| Is it safe to hold an object reference across a transaction? | `object-lifecycle-and-gc` |
