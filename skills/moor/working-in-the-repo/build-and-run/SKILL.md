...
when_to_use = "Use when you must compile mooR, pick the fastest correct build for the area you changed, or bring a server up locally with telnet and web access. Use it when a build is slow, when the stack will not start, when the database will not import, or when you must know what processes a running system consists of. Do not use it to choose or write tests; that is the testing skill. Do not use it for style rules or pull-request norms; that is conventions. Not for the Torchship game database, for authoring MOO verbs inside a running world, or for Thetis's own internals."
universal = false
...
time; that is why release binaries are large. `performance-and-profiling` owns
how to measure.
...
tracing; `performance-and-profiling` says when that is worth doing. Consult a
...
| Deployment examples | The trees under `deploy/` | Clustered, Kubernetes, Debian package and TLS shapes. Not a development loop; see `deployment-and-release` |
...
of the split. `moor/services/daemon-and-rpc` explains the enrolment and
...
| A `.fbs` edit changes nothing | Rust bindings are committed, not generated by `build.rs` | Regenerate the bindings and commit them. See `moor/services/wire-schema` |
...
| A performance number looks wrong | You measured a `dev` build | Rebuild with `--release` and measure again. See `performance-and-profiling` |

## Read first / read next

Read `repo-tooling` for what `bacon`, `process-compose`, `moorc` and the
`scripts/` wrappers actually are. Read `testing` before you assert that a change
works, and `performance-and-profiling` if the claim is about speed. Read
`deployment-and-release` for the container and package shapes, and
`moor/content-pipeline/cores-and-bootstrap` before you change import or startup
behaviour.
