#!/usr/bin/env bash
# Copies the canonical github.rs into every other git-* tool crate.
#
...
#
# git-whoami holds the canonical copy. Edit that one, then run this.
set -euo pipefail
cd "$(dirname "$0")/.."

SRC=tools/git-whoami/src/github.rs
test -f "$SRC" || { echo "missing $SRC" >&2; exit 1; }

for dir in tools/git-*/; do
  [ "$dir" = "tools/git-whoami/" ] && continue
