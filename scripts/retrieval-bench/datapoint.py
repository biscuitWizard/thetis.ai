...
def write_readme(staging, point, skills, route, toolret):
    ...
    lines += [
        ...
        "loads straight into a dataframe. Filter on `schema` first: the earliest",
        "rows are schema 1, written by CI before the ablation harness existed, and",
        "carry nine fields rather than this run's eighty-odd. They share a file, not",
        "a shape.",
        "",
        "```python",
        "import pandas as pd",
        "df = pd.read_json('results.jsonl', lines=True)",
        f"df = df[df['schema'] == {SCHEMA}]   # older rows measured something else",
        "df.plot(x='measured_at', y=['route_tags_f1', 'route_hybrid_f1'])",
        "```",
        "",
        "Two more fields decide whether two rows are comparable: `mode` (dense or a BM25",
